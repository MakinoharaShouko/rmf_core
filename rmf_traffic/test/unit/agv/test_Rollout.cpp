...
SCENARIO("Test Rollout on graph with side routes")
{
  ...
  GIVEN("Nav graph with only a pullover")
  {
    ...
    graph.add_waypoint(test_map_name, {-5.0, 0.0, 0}); // 0
    graph.add_waypoint(test_map_name, { 0.0, 0.0, 0}); // 1
    graph.add_waypoint(test_map_name, { 5.0, 0.0, 0}); // 2
    graph.add_waypoint(test_map_name, { 0.0, 5.0, 0}).set_holding_point(true); // 3
